...
class CustomerCreate{
    public static async createCustomer(sqlConfirm: string, sqlCreate: string, parameter: any, res: Response): Promise<any> {
        await pool.task(async consult => {
            ...
            .then((response) => {
                if (response?.id_customer != 0) {
                    res.status(200).json({ answer: 'Create customer', newCode: response?.id_customer });
                } else {
